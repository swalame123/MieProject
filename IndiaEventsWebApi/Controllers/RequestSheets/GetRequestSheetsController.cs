...
namespace IndiaEventsWebApi.Controllers.RequestSheets
{
    ...
    public class GetRequestSheetsController : ControllerBase
    {
            ...
            try
            {
                ...
                List<ProcessSheetPayload> eventRequestBrandsList = sheetData
                    .Select(row => new ProcessSheetPayload
                    {
                        ...
                        EventEndDate = row.TryGetValue("End Date", out var eventEndDate) ? eventEndDate?.ToString() : null,
                        ...
                        TotalBudget = row.TryGetValue("Budget Amount", out var totalBudget) && double.TryParse(totalBudget?.ToString(), out var parsedTotalBudget) ? parsedTotalBudget : 0,
            ...
            if (targetRow != null)
            {
                ...
                List<HonorariumPayload> eventRequestBrandsList = sheetData
                    ...
                    .Select(row => new HonorariumPayload
                    {
                        ...
                        EventOpen30days = row.TryGetValue("EventOpen30days", out var eventOpen30days) ? eventOpen30days?.ToString() : null,
                        EventWithin7days = row.TryGetValue("EventWithin7days", out var eventWithin7days) ? eventWithin7days?.ToString() : null,
                        CreatedOn = row.TryGetValue("Created On", out var premarketingHeadApproval) ? premarketingHeadApproval?.ToString() : null,
                        CreatedDateHelper = row.TryGetValue("Created Date - Helper", out var createdDateHelper) ? createdDateHelper?.ToString() : null,
                        ...
                        JVNo = row.TryGetValue("JV Number", out var eventRequestStatus) ? eventRequestStatus?.ToString() : null,
                        JVDate = row.TryGetValue("JV Date", out var eventApprovedDate) ? eventApprovedDate?.ToString() : null,
                        PVNo = row.TryGetValue("PV Number", out var miplInvitees) ? miplInvitees?.ToString() : null,
                        ...
                        BrandName = row.TryGetValue("Brand Name", out var prefBExpenseExcludingTaxApproval) ? prefBExpenseExcludingTaxApproval?.ToString() : null,
            ...
            if (targetRow != null)
            {
                ...
                List<DeviationDataPayload> eventRequestBrandsList = sheetData
                    ...
                    .Select(row => new DeviationDataPayload
                    {
                        ...
                        TotalBudget = row.TryGetValue("Budget Amount", out var totalBudget) && int.TryParse(totalBudget?.ToString(), out var parsedTotalBudget) ? parsedTotalBudget : 0,
                        CostperparticipantHelper = row.TryGetValue("Cost per participant - Helper", out var _7daysSalesHeadApprovaldate) ? _7daysSalesHeadApprovaldate?.ToString() : null,
            ...
            if (targetRow != null)
            {
                ...
                List<EventSettlementPayload> eventRequestBrandsList = sheetData
                    ...
                    .Select(row => new EventSettlementPayload
                    {
                        ...
                        InviteesParticipated = row.TryGetValue("Invitees Participated", out var honorariumApprovedDate) ? honorariumApprovedDate?.ToString() : null,
                        ExpenseDetails = row.TryGetValue("Expense Details", out var ExpenseDetails) ? ExpenseDetails?.ToString() : null,
                        TotalExpenseDetails = row.TryGetValue("Total Expense Details", out var TotalExpenseDetails) ? TotalExpenseDetails?.ToString() : null,
                        AdvanceDetails = row.TryGetValue("Advance Details", out var AdvanceDetails) ? AdvanceDetails?.ToString() : null,
                        ...
                        TotalExpense = row.TryGetValue("Total Expense", out var totalExpense) && double.TryParse(totalExpense?.ToString(), out var parsedTotalExpense) ? parsedTotalExpense : 0,
                        TotalBudget = row.TryGetValue("Budget Amount", out var totalBudget) && double.TryParse(totalBudget?.ToString(), out var parsedTotalBudget) ? parsedTotalBudget : 0,
                        TotalActual = row.TryGetValue("Budget Amount", out var TotalActual) && double.TryParse(TotalActual?.ToString(), out var parsedTotalActual) ? parsedTotalActual : 0,
        ...
        public IActionResult GetAttachmentsFromHonorariumSheetBasedOnEventId(string eventId)
        {
            ...
            if (targetRow != null)
            {
                var attachments = SheetHelper.GetAttachmentsIdForRow(smartsheet, sheet.Id.Value, targetRow.Id.Value);
                ProductBrandsListrowData["Attachments"] = attachments;
            }
            return Ok(ProductBrandsListrowData);
        }
        ...
        public IActionResult GetAmountsFromAdvanceProvidedColumnUsingEventId(string eventId)
        {
            string sheetId = configuration.GetSection("SmartsheetSettings:EventSettlement").Value;
            Sheet sheet = SheetHelper.GetSheetById(smartsheet, sheetId);
            Column targetColumn = sheet.Columns.FirstOrDefault(column => string.Equals(column.Title, "Advance Details", StringComparison.OrdinalIgnoreCase));
